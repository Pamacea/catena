...
export function PrayerCard({
  ...
  if (variant === "compact") {
    return (
      <Link
        href={`/liturgy/${slug}`}
        className="flex items-center gap-3 p-3 rounded-lg hover:bg-parchment-100 dark:hover:bg-stone-700 transition-colors border border-transparent hover:border-parchment-200 dark:hover:border-stone-700 group"
      >
        <span className="text-2xl">{config.icon}</span>
        <div className="flex-1 min-w-0">
          <p className="font-medium text-ink-800 dark:text-ink-200 group-hover:text-gold-700 dark:group-hover:text-gold-500 transition-colors">
            {name}
          </p>
          {excerpt.french && <p className="text-xs text-ink-500 dark:text-ink-500 truncate">{excerpt.french}</p>}
        </div>
        <svg
          className="w-5 h-5 text-ink-400 dark:text-ink-500 group-hover:text-gold-600"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
  ...
  if (variant === "detailed") {
    return (
      <Link
        href={`/liturgy/${slug}`}
        className="block p-6 rounded-lg border border-parchment-200 dark:border-stone-700 bg-white dark:bg-stone-800 hover:shadow-lg transition-all"
      >
        {/* En-tête */}
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          <span className="px-2 py-1 bg-parchment-100 dark:bg-stone-700 text-ink-700 dark:text-ink-300 rounded text-xs">
            {config.label}
          </span>
          {tags?.slice(0, 3).map(tag => (
            <span key={tag} className="px-2 py-1 bg-parchment-50 dark:bg-stone-800/50 text-ink-600 dark:text-ink-400 rounded text-xs">
              #{tag}
            </span>
          ))}
        </div>
        ...
        {/* Description */}
        {description && <p className="text-ink-600 dark:text-ink-400 text-sm mb-4">{description}</p>}

        {/* Extrait bilingue */}
        <div className="bg-parchment-50 dark:bg-stone-800/50 rounded-lg p-4 border border-parchment-200 dark:border-stone-700">
          {excerpt.latin && (
            <p className="text-ink-600 dark:text-ink-400 italic font-serif text-sm mb-2">{excerpt.latin}</p>
          )}
          <p className="text-ink-800 dark:text-ink-200 font-serif">{excerpt.french}</p>
        </div>

        {/* Toggle Latin (non cliquable dans ce cas) */}
        {excerpt.latin && controlledShowLatin === undefined && (
          <div className="flex items-center justify-end mt-3 gap-2">
            <span className="text-xs text-ink-500 dark:text-ink-500">Latin:</span>
            <button
              onClick={e => {
                e.preventDefault();
              ...
              className={cn(
                ...
                showLatin ? "bg-liturgical-purple" : "bg-parchment-200 dark:bg-stone-600"
              )}
            >
              <span
                className={cn(
                  "absolute top-1 w-4 h-4 bg-white dark:bg-stone-300 rounded-full shadow transition-all",
  ...
  return (
    <Link
      href={`/liturgy/${slug}`}
      className="block p-4 rounded-lg border border-parchment-200 dark:border-stone-700 bg-white dark:bg-stone-800 hover:shadow-md transition-all group"
    >
      <div className="flex items-start gap-3">
        <span className="text-2xl">{config.icon}</span>

        <div className="flex-1 min-w-0">
          <h3 className="font-serif font-bold text-ink-900 dark:text-parchment-50 group-hover:text-gold-600 transition-colors">
            {name}
          </h3>
          {nameLatin && <p className="text-sm text-ink-500 dark:text-ink-500 italic">{nameLatin}</p>}

          {/* Extrait */}
          <div className="mt-2 space-y-1">
            {excerpt.latin && showLatin && (
              <p className="text-sm text-ink-600 dark:text-ink-400 italic truncate">{excerpt.latin}</p>
            )}
            <p className="text-sm text-ink-700 dark:text-ink-300 line-clamp-2">{excerpt.french}</p>
          </div>
          ...
          {tags && tags.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {tags.slice(0, 2).map(tag => (
                <span
                  key={tag}
                  className="text-xs text-ink-500 dark:text-ink-500 bg-parchment-50 dark:bg-stone-800/50 px-2 py-0.5 rounded"
                >
                  #{tag}
                </span>
              ))}
...
export function PrayerMention({ slug, name, icon = "📜" }: PrayerMentionProps) {
  return (
    <Link
      href={`/liturgy/${slug}`}
      className="inline-flex items-center gap-1 px-2 py-1 bg-parchment-100 dark:bg-stone-700 hover:bg-parchment-200 dark:hover:bg-stone-600 rounded text-sm text-ink-700 dark:text-ink-300 hover:text-gold-600 transition-colors"
    >
      <span>{icon}</span>
      <span>{name}</span>
    </Link>
...
export function RosaryDecade({ mysteryName, prayers }: RosaryDecadeProps) {
  return (
    <div className="bg-parchment-50 dark:bg-stone-800/50 rounded-lg p-4 border border-parchment-200 dark:border-stone-700">
      <h4 className="text-sm font-semibold text-ink-700 dark:text-ink-300 uppercase tracking-wider mb-3">
        {mysteryName}
      </h4>
      ...
      <div className="space-y-3">
        ...
        {prayers.ourFather && (
          <div className="text-center">
            <p className="text-xs text-ink-400 dark:text-ink-500 uppercase mb-1">Notre Père</p>
            <p className="font-serif text-ink-700 dark:text-ink-300 text-sm">{prayers.ourFather.french}</p>
          </div>
        )}

        {/* Je vous salue Marie (x10) */}
        <div className="space-y-2">
          <p className="text-xs text-ink-400 dark:text-ink-500 uppercase text-center">Je vous salue Marie (10)</p>
          {prayers.hailMarys.map((hailMary, i) => (
            <p key={i} className="font-serif text-ink-700 dark:text-ink-300 text-sm text-center italic">
              {hailMary.french}
            </p>
          ))}
        </div>
        ...
        {/* Gloire au Père */}
        {prayers.gloryBe && (
          <div className="text-center pt-2 border-t border-parchment-200 dark:border-stone-700">
            <p className="font-serif text-ink-700 dark:text-ink-300 text-sm">{prayers.gloryBe.french}</p>
          </div>
        )}
        ...
        {prayers.fatimaPrayer && (
          <div className="text-center pt-2 border-t border-parchment-200 dark:border-stone-700">
            <p className="font-serif text-ink-700 dark:text-ink-300 text-sm italic">{prayers.fatimaPrayer.french}</p>
          </div>
        )}
      </div>
    </div>
    ...
    <div className={cn("rounded-lg border-2 p-4", colorClasses[color])}>
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-sm text-ink-600 dark:text-ink-400">{date}</p>
        </div>
        <div
          className={cn(
        ...
        {prayers.map(prayer => (
          <li key={prayer.slug}>
            <Link
              ...
              className="text-sm text-ink-700 dark:text-ink-300 hover:text-gold-600 flex items-center gap-2 py-1"
            >
              <span>📜</span>
