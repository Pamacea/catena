  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <header className="text-center space-y-4 pb-8 border-b border-gold-400/40">
        ...
        {prayer.description && (
          <p className="text-ink-600 mx-auto">{prayer.description}</p>
        )}

        {/* Métadonnées */}
        <div className="flex items-center justify-center gap-4 text-sm text-gold-600">
        ...
        {prayer.sections.map((section, sectionIndex) => {
          ...
          return (
            <section key={sectionIndex} className="space-y-4">
              {section.title && (
                <h3 className="font-serif text-xl font-semibold text-ink-900 text-center">
                  {section.title}
                </h3>
              )}
      ...
      {relatedPrayers.length > 0 && (
        <aside className="pt-8 border-t border-gold-400/40">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {relatedPrayers.map(related => (
              <Link
                ...
                className="block p-4 rounded-xs border border-gold-400/40 bg-gold-50/30/30 hover:bg-parchment-100 transition-colors"
              >
                <p className="font-serif font-semibold text-ink-900">
                  {related.name}
                </p>
                {related.nameLatin && (
                  <p className="text-sm text-ink-600 italic truncate">
                    {related.nameLatin}
                  </p>
                )}
              </Link>
      ...
      {/* Retour */}
      <div className="pt-4 border-t border-gold-400/40/50/50">
        <Link
          href="/liturgy"
